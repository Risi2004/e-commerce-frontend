...
function KeyboardPage() {
  ...
  const uniqueBrands = [...new Set(keyboards.map((item) => item.brand))];

  const handleAddToCart = (item) => {
    addToCart({ ...item, type: "Keyboard" });
        ...
        <div className="relative max-w-7xl mx-auto text-center">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-white/10 backdrop-blur-lg rounded-2xl mb-6">
            <svg
              ...
              aria-hidden="true"
            >
              <path d="M4 5h16v3H4zm0 5h16v4H4zm0 6h16v3H4z" />
            </svg>
          </div>
          ...
          <p className="text-xl text-indigo-200 max-w-2xl mx-auto">
            Find your perfect mechanical or membrane keyboard.
          </p>
          <div className="mt-6 flex items-center justify-center gap-4 text-indigo-200">
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
              <span className="text-sm">{filteredKeyboards.length} Products Available</span>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 p-4 mb-6 w-full">
        <img
          ...
          src="/images/accessories/Redragon K552 Kumara LED RGB.jpg"
          alt={item.name}
      ...
      <p className="text-gray-600 mb-2 text-center">
        <strong>{item.specs?.SwitchType}</strong> • <strong>{item.specs?.Layout}</strong>
      </p>
      ...
      {filteredKeyboards.length === 0 && (
        <p className="text-center text-gray-500 mt-10">No keyboards found.</p>
      )}
    </div>
  );
}
